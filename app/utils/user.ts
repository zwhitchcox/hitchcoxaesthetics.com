...
export function useIsProvider(
	user: Pick<ReturnType<typeof useUser>, 'type'> | null,
) {
	return user?.type === 'provider'
}

const userTypes = ['provider', 'client', 'admin'] as const
const UserTypeSchema = z.enum(userTypes)
export type UserType = z.infer<typeof UserTypeSchema>

export function validatePhoneNumber(number: unknown): number is string {
	return typeof number === 'string' && number.length === 9
}
